...
def train(config):
    ...
    dehaze_net = net.dehaze_net()  # Remove .cuda() to run on CPU
    ...
    criterion = nn.MSELoss()  # Remove .cuda() to run on CPU
    ...
    for epoch in range(config.num_epochs):
        for iteration, (img_orig, img_haze) in enumerate(train_loader):
            ...
            img_orig = img_orig  # Remove .cuda() to run on CPU
            img_haze = img_haze  # Remove .cuda() to run on CPU
            ...
            if ((iteration+1) % config.display_iter) == 0:
                print("Epoch [{}/{}], Iteration [{}/{}], Loss: {:.4f}".format(epoch+1, config.num_epochs, iteration+1, len(train_loader), loss.item()))
